...
//        conf.set(ExecutionCheckpointingOptions.ENABLE_CHECKPOINTS_AFTER_TASKS_FINISH, false);
        ...
        // TODO 开启 Changelog
        // 要求： checkpoint的最大并发必须为1， 其他参数建议在flink-conf文件中去指定
        ...
        // 5、最小等待间隔： 上一轮checkpoint结束 到 下一轮checkpint开始 之间的间隔，设置了>0,并发就会变成1
        checkpointConfig.setMinPauseBetweenCheckpoints(1000);
        // 6、取消作业时，checkpoint的数据， 是否保留在外部系统
        // DELETE_ON_CANCELLATION: 主动cancel时， 删除存在外部系统的chk-xx目录 （如果是程序突然挂断， 则不会删除）
        // RETURN_ON_CALCELLATION: 主动cancel时， 外部系统的chk-xx目录会保存下来
        checkpointConfig.setExternalizedCheckpointCleanup(CheckpointConfig.ExternalizedCheckpointCleanup.RETAIN_ON_CANCELLATION);
        // 7、允许 checkpoint  连续失败的次数， 默认0--》 表示checkpoint一旦失败， job就会挂掉
        checkpointConfig.setTolerableCheckpointFailureNumber(10);

        // TODO 开启  非对齐检查点（barrier非对齐）
        // 开启的要求： Checkpoint必须是精准一次，最大并发必须设为1
 ...
 *      精准一次:在barrier对产过程中，barrier后面的数据 阻塞等待(不会越过barrier)
