...
//        sensorDS.windowAll()
        ...
        // 基于时间的
//        sensorKS.window(SlidingProcessingTimeWindows.of(Time.seconds(10), Time.seconds(2))) // 滑动窗口， 窗口长度为10s, 滑动步长2s
...
//        sensorKS.window(GlobalWindows.create())  // 全局窗口， 计数窗口的底层就是用这个， 需要自定义的时候才会用

        // TODO 2. 指定 窗口函数：  窗口内数据的 计算逻辑
        WindowedStream<WaterSensor, String, TimeWindow> sensorWS = sensorKS.window(TumblingProcessingTimeWindows.of(Time.seconds(10)));// 滚动窗口， 窗口长度为10s
